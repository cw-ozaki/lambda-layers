...
```bash
$ curl -sSL https://github.com/cw-ozaki/lambda-layers/releases/download/awscli%2F0.0.1/awscli.zip | aws lambda publish-layer-version --layer-name "shell-runtime" --description "Layer to add awscli command to AWS Lambda" --zip-file fileb:///dev/stdin
{
    "Content": {
