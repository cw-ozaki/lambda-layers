...
```bash
$ curl -sSL https://github.com/cw-ozaki/lambda-layers/releases/download/jq%2F0.0.1/jq.zip | aws lambda publish-layer-version --layer-name "jq" --description "Layer to add jq command to AWS Lambda" --zip-file fileb:///dev/stdin
{
    "Content": {
